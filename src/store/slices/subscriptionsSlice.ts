...
export const fetchSubscriptions = createAsyncThunk(
  ...
  async ({ page = 1, pageSize = 10 }: { page?: number; pageSize?: number } = {}) => {
    const response = await subscriptionsApi.getAll(page, pageSize);
    return response;
  }
);
...
const subscriptionsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(fetchSubscriptions.fulfilled, (state, action: PayloadAction<PaginatedResponse<Subscription>>) => {
